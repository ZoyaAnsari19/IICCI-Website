export const Stats = () => {
  ...
  return (
    <section
      id="stats"
      className="relative section-padding overflow-hidden border-y border-white/5 z-10"
    >
      <div className="absolute inset-0 bg-grid bg-grid-fade opacity-30 pointer-events-none" />
      <div className="absolute -top-32 left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full bg-gold/5 blur-3xl pointer-events-none" />
        ...
        <ul className="stats-grid list-none p-0 m-0">
          {stats.map((stat) => (
            <li key={stat.label} className="min-w-0">
              <article className="stats-card relative group glass-dark rounded-2xl p-5 sm:p-6 lg:p-7 card-lift border border-white/5 reveal-up h-full flex flex-col">
                <div className="mb-4">
                ...
                <div className="mt-auto min-w-0">
                  <div className="flex flex-wrap items-baseline gap-x-1 gap-y-0 min-w-0">
                    <span
                      className="stat-value text-white tabular-nums counter break-all sm:break-normal"
                  ...
                  <p className="text-sm sm:text-base text-white font-semibold mt-2 leading-snug">{stat.label}</p>
