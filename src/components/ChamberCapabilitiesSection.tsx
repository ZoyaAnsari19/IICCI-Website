...
export const ChamberCapabilitiesSection = () => {
  return (
    ...
    >
      {/* Deep blue cinematic backdrop (matches Training section vibe) */}
      <div className="absolute inset-0 bg-gradient-to-br from-navy-950 via-royal-dark to-navy-950" />
      <div className="absolute inset-0 bg-grid opacity-20 pointer-events-none" />
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-[420px] h-[420px] rounded-full bg-gold/10 blur-[120px] animate-pulse" />
        <div
          className="absolute bottom-1/4 right-1/4 w-[520px] h-[520px] rounded-full bg-royal/20 blur-[120px] animate-pulse"
      ...
      <div className="relative max-w-[1400px] mx-auto px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-12 lg:mb-14 reveal-up">
          ...
          >
            <span className="text-white">Trusted services for</span>{" "}
        ...
        <ul className="grid md:grid-cols-3 gap-5 lg:gap-6 list-none p-0 m-0">
          {chamberCapabilities.map((item) => (
            <li key={item.id} className="min-w-0 reveal-up">
              <article className="h-full flex flex-col rounded-3xl p-7 lg:p-8 glass-dark border border-white/10 card-lift shadow-premium hover:shadow-gold transition-shadow">
                ...
                <p className="text-sm text-white/65 leading-relaxed flex-1">{item.description}</p>

                {item.href && item.cta ? (
                  <Link
                    href={item.href}
                    className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-white/85 hover:text-gold transition group"
                  >
                    {item.cta}
                    <i
