    ...
    <section id="services" className="relative section-padding overflow-hidden">
      ...
      <div className="relative max-w-[1400px] mx-auto px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div className="max-w-2xl reveal-up">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass mb-4">
              ...
              <span className="text-[10px] uppercase tracking-[0.25em] text-white/70">Our Services</span>
            </div>
            <h2 className="display-title font-display font-bold mb-4">
              <span className="text-white">Enterprise-grade services for</span>{' '}
          ...
          <a href="#contact" className="inline-flex items-center gap-3 px-6 py-3 rounded-full glass border border-white/10 text-white text-xs font-semibold hover:border-gold/40 transition self-start reveal-up">
            Talk to an expert
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5">
          {services.map((s, i) => (
            <div key={s.title}
              className="service-card group glass-dark rounded-2xl p-6 lg:p-7 border border-white/5 card-lift cursor-pointer reveal-up"
            ...
            >
              <div className="flex items-start justify-between mb-5">
                ...
                <span className="text-[9px] uppercase tracking-[0.2em] text-white/40 px-2 py-1 rounded-full border border-white/10">{s.tag}</span>
              </div>
              <h3 className="text-lg font-display font-bold text-white mb-2 group-hover:text-gold transition">{s.title}</h3>
              <p className="text-sm text-white/60 leading-relaxed mb-5">{s.desc}</p>
              <div className="flex items-center justify-between text-xs">
                <span className="text-white/40 group-hover:text-gold transition flex items-center gap-2">
                  Learn more
                  <i className="fas fa-arrow-right text-[10px] transition-all group-hover:translate-x-1"></i>
                </span>
                <i className="fas fa-up-right-from-square text-white/30 group-hover:text-gold transition"></i>
              </div>
            </div>
          ))}
        </div>
